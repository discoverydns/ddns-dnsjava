 ...
 * @param alg The original key algorithm.
 * @param digestid The digest id code.
 * @param digest The hexadecimal textual representation of the original key.
 * @throws IOException In case of an invalid provided textual representation.
 */
public
DSRecord(Name name, int dclass, long ttl, int footprint, int alg,
		int digestid, String digest) throws IOException
{
	super(name, Type.DS, dclass, ttl);
	this.footprint = checkU16("footprint", footprint);
	this.alg = checkU8("alg", alg);
	this.digestid = checkU8("digestid", digestid);
	this.digest = new Tokenizer(digest).getHex();
}
...
public String
		getTextualDigest() {
	return (digest != null? base16.toString(digest) : "");
}
