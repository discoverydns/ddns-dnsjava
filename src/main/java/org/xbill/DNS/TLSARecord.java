 ...
 * that will be matched against the association data. 
 * @param matchingType How the certificate association is presented.
 * @param certificateAssociationData The hexadecimal textual representation of the "certificate association data" to be
 * matched.
...
TLSARecord(Name name, int dclass, long ttl, 
		int certificateUsage, int selector, int matchingType,
		String certificateAssociationData) throws IOException
{
	super(name, Type.TLSA, dclass, ttl);
	this.certificateUsage = checkU8("certificateUsage", certificateUsage);
	this.selector = checkU8("selector", selector);
	this.matchingType = checkU8("matchingType", matchingType);
	this.certificateAssociationData = new Tokenizer(certificateAssociationData).getHex();
	if (this.certificateAssociationData.length > 0xFFFF) {
		throw new IllegalArgumentException("\"certificateAssociationData\" array " +
...
public final String
	...
	return base16.toString(certificateAssociationData);
}
